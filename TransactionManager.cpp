...
bool TransactionManager::hasCycleWithRWCheck(int startNode)
{
        ...
        if (nodeConflicts != conflicts.end())
        {
            ...
            for (const auto &edge : nodeConflicts->second)
            {
                if (transactionGraph.count(edge.first) || edge.first == startNode)
                {
                    ...
                    // Check for more than two consecutive RW edges
                    // if (newRWCount >= 2)
                    // {
                    //     return true;
                    // }
...
// bool TransactionManager::checkCycle(int tid)
// {
//     bool ifCycle = false;
//     bool twoRWedges = false;
//     int curRWedges = 0;
//     int curTID = tid;

//     bool nextTransactionInGraph = false;
//     for (auto nextTID : conflicts[curTID])
//     {
//         if (transactionGraph.find(nextTID.first) != transactionGraph.end())
//         {
//         }
//     }
//     do
...
//     for (int graphTID = 0; graphTID < transactionGraph.size(); graphTID++)
//     {
//     }
...
void TransactionManager::transactionWrite(int tid, int var, int val, Operation op)
{
    ...
    if (!replicated)
    {
        ...
        else
        {
            ...
            // {
            //     st->snapshots[tid] = st->committedData;
            // }
...
void TransactionManager::dump()
{
    ...
    {
        // TODO getID() function to be added as siteID private variable
...
void TransactionManager::transactionEnd(int tid, int endTime)
{
    ...
    // check for site failure

    // check for RW-Cycle
    if (hasCycleWithRWCheck(tid))
    {
        canCommit = false;
    ...
    if (canCommit)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (st->status == SiteStatus::UP)
            {
                ...
                // st->committedData = st->snapshots[tid]; //not all sites will have a snapshot of transaction T
...
void TransactionManager::wrapUp()
{
